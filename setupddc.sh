...
echo "Get Swarm manager token"
docker-machine ssh ucp0 docker swarm join-token -q manager > managertoken
echo "Get Swarm worker token"
docker-machine ssh ucp0 docker swarm join-token -q worker > swarmtoken
echo "--------- Swarm token: $(cat swarmtoken) and Manager token: $(cat managertoken)"

echo "Creating manager nodes"
for (( COUNT=1; COUNT \< $1; COUNT++))
do
    echo "Creating UCP node $COUNT"
    docker-machine create -d virtualbox --virtualbox-memory "4096" ucp$COUNT && docker-machine ssh ucp$COUNT docker swarm join --token $(cat managertoken) $(docker-machine ip ucp0)
done
echo "----------- Installing UCP ------------------"
...
do
	...
	docker-machine create -d virtualbox --engine-label environment=TESTLABEL node$COUNT  && echo "Created node$COUNT" && echo node$COUNT >> swarmnodes && docker-machine ssh node$COUNT docker swarm join --token $(cat swarmtoken) $(docker-machine ip ucp0)
done
...
docker-machine ssh dtr0 docker swarm join --token $(cat swarmtoken) $(docker-machine ip ucp0)
echo "--------------- Installing DTR -----------"
# Install DTR
...
do
    ...
    docker-machine create -d virtualbox --virtualbox-memory "3072" dtr$COUNT && docker-machine ssh dtr$COUNT docker swarm join --token $(cat swarmtoken) $(docker-machine ip ucp0)
